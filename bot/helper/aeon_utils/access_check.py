    ...
    if message.chat.type != message.chat.type.BOT:
        ...
        if not token_timeout or user_id in {
            ...
            try:
                ...
                button.data_button(
                    "Start",
                    f"aeon {user_id} private",
                    "header",
                )

                msg.append(
                    "You haven't initiated the bot in a private message!"
                )
...
async def is_paid(user_id):
    """
    Checks if a user is considered 'paid' by verifying membership
    in PAID_CHANNEL_ID.
    ...
    Returns:
        True if the user is considered paid or if PAID_CHANNEL_ID
...
async def token_check(user_id, button=None):
    ...
    Returns:
        A tuple (message_string, button_maker_instance)
        or (None, None).
    """
    token_timeout = Config.TOKEN_TIMEOUT
    ...
    if isExpired:
        ...
        if Config.PAID_CHANNEL_ID and Config.PAID_CHANNEL_LINK:
            msg += (
                " or subscribe to the paid channel "
                "for token-free access."
            )
